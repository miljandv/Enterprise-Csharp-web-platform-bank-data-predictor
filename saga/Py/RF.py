...
def potrosnja(customerId):
    ...
    plt.cla()
def gotovinski_predlozi():
    f = open(r"C:\Users\milja\source\repos\saga\saga\Output\stanbeni.txt", "a")
    verovatnoce = model_got.predict_proba(X_test)
    verovatnoce = (verovatnoce[:, 1]).reshape(-1,1);
    print(verovatnoce.shape)
    customerIds = (dataset_got[42700:, 0]).reshape(-1,1);
    verovatnoce = np.hstack((customerIds, verovatnoce))
    verovatnoce = np.sort(verovatnoce, axis=0)
    verovatnoce = np.flip(verovatnoce, 0)
    verovatnoce = verovatnoce[:20,:]
    for i in range(len( verovatnoce)):
        f.write(verovatnoce[i][0].astype(int).astype(str))
        f.write(' ')
        f.write((verovatnoce[i][1].astype(str)))
        f.write('\n')
    f.close()
def stambeni_predlozi():
    f = open(r"C:\Users\milja\source\repos\saga\saga\Output\stanbeni.txt", "a")
    verovatnoce = model_stam.predict_proba(X_test)
    verovatnoce = (verovatnoce[:, 1]).reshape(-1,1)
    print(verovatnoce.shape)
    customerIds = (dataset_stam[42700:, 0]).reshape(-1,1)
    verovatnoce = np.hstack((customerIds, verovatnoce))
    verovatnoce = np.sort(verovatnoce, axis=0)
    verovatnoce = np.flip(verovatnoce, 0)
    verovatnoce = verovatnoce[:10,:]
